...
class PolicyNode(base.ESILEAPObject):
    ...
    def get_all_by_project_id(cls, context, project_id):
        db_policy_nodes = cls.dbapi.policy_node_get_all_by_project_id(context, project_id)
        return cls._from_db_object_list(context, db_policy_nodes)


    @classmethod
    def get_all_by_request_project_id(cls, context, project_id):
        db_policy_nodes = cls.dbapi.policy_node_get_all_by_request_project_id(context, project_id)
        return cls._from_db_object_list(context, db_policy_nodes)


    @classmethod
    def get_all_by_policy_uuid(cls, context, policy_uuid):
        db_policy_nodes = cls.dbapi.policy_node_get_all_by_policy_uuid(context, policy_uuid)
        return cls._from_db_object_list(context, db_policy_nodes)


    @classmethod
    def get_all_by_request_uuid(cls, context, request_uuid):
        db_policy_nodes = cls.dbapi.policy_node_get_all_by_request_uuid(context, request_uuid)
        return cls._from_db_object_list(context, db_policy_nodes)
    ...
    def save(self, context=None):
        updates = self.obj_get_changes()
        db_policy_node = self.dbapi.policy_node_update(context, self.node_uuid,  updates)
        self._from_db_object(context, self, db_policy_node)
