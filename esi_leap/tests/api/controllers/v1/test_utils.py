...
class TestGetObjectUtils(testtools.TestCase):
    ...
    @mock.patch("oslo_utils.uuidutils.is_uuid_like")
    @mock.patch("esi_leap.objects.offer.Offer.get")
    def test_get_offer_uuid_bad_status(self, mock_offer_get, mock_is_uuid_like):
        mock_is_uuid_like.return_value = True
        mock_offer_get.return_value = test_offer

        self.assertRaises(
            exception.OfferNotFound, utils.get_offer, test_offer.uuid, statuses.DELETED
        )
    ...
    def test_get_offer_name_available(self, mock_offer_get_all, mock_is_uuid_like):
    ...
    @mock.patch("oslo_utils.uuidutils.is_uuid_like")
    @mock.patch("esi_leap.objects.lease.Lease.get")
    def test_get_lease_uuid_bad_status(self, mock_lease_get, mock_is_uuid_like):
        mock_is_uuid_like.return_value = True
        mock_lease_get.return_value = test_lease

        self.assertRaises(
            exception.LeaseNotFound, utils.get_lease, test_lease.uuid, statuses.DELETED
        )
    ...
    def test_get_lease_name_active(self, mock_lease_get_all, mock_is_uuid_like):
        mock_is_uuid_like.return_value = False
        ...
        mock_lease_get_all.assert_called_once_with(
            {"name": test_lease.name, "status": statuses.ACTIVE}
        )
...
class TestCheckResourceAdminUtils(testtools.TestCase):
    ...
    def test_check_resource_admin_owner(self, mock_authorize, mock_gopi):
        ...
        utils.check_resource_admin(
            owner_ctx.to_policy_values(), test_node_1, test_offer.project_id
        )
    ...
    def test_check_resource_admin_admin(self, mock_authorize, mock_gopi):
        ...
        utils.check_resource_admin(
            admin_ctx.to_policy_values(), test_node_2, bad_test_offer.project_id
        )
    ...
    @mock.patch.object(test_node_2, "get_owner_project_id")
    @mock.patch("esi_leap.api.controllers.v1.utils.resource_policy_authorize")
    def test_check_resource_admin_invalid_owner(self, mock_authorize, mock_gopi):
        mock_gopi.return_value = owner_ctx_2.project_id
        ...
        self.assertRaises(
            exception.HTTPResourceForbidden,
            utils.check_resource_admin,
...
class TestCheckResourceLeaseAdminUtils(testtools.TestCase):
    ...
    @mock.patch("esi_leap.objects.lease.Lease.get")
    @mock.patch.object(test_node_1, "get_lease_uuid", return_value="a-lease-uuid")
    @mock.patch.object(
        test_node_1, "get_lessee_project_id", return_value=test_offer.project_id
    )
    def test_check_resource_lease_admin_okay(self, mock_glpi, mock_glu, mock_lease_get):
    ...
    @mock.patch.object(test_node_1, "get_lease_uuid", return_value="a-lease-uuid")
    @mock.patch.object(
        test_node_1, "get_lessee_project_id", return_value="other-lessee"
    )
    def test_check_resource_lease_admin_not_lessee(
        self, mock_glpi, mock_glu, mock_lease_get
    ):
        mock_lease_get.return_value = self.test_lease
        parent_lease_uuid = utils.check_resource_lease_admin(
            owner_ctx.to_policy_values(),
    ...
    @mock.patch("esi_leap.objects.lease.Lease.get")
    @mock.patch.object(test_node_1, "get_lease_uuid", return_value=None)
    @mock.patch.object(
        test_node_1, "get_lessee_project_id", return_value=test_offer.project_id
    )
    def test_check_resource_lease_admin_not_lease(
        self, mock_glpi, mock_glu, mock_lease_get
    ):
        mock_lease_get.return_value = self.test_lease
        parent_lease_uuid = utils.check_resource_lease_admin(
    ...
    def test_check_resource_lease_admin_parent_lease(
        self, mock_glpi, mock_glu, mock_lease_get
    ):
    ...
    def test_check_resource_lease_admin_bad_time(
        self, mock_glpi, mock_glu, mock_lease_get
    ):
        mock_lease_get.return_value = self.test_lease
        self.assertRaises(
            exception.ResourceNoPermissionTime,
...
class TestOfferPolicyAndRetrieveUtils(testtools.TestCase):
    ...
    def test_check_offer_policy(self, mock_get_offer, mock_authorize):
        ...
        mock_authorize.assert_called_with(
            "test_policy:test",
            target,
            admin_ctx.to_policy_values(),
            "offer",
...
class TestOfferLesseeUtils(testtools.TestCase):
    ...
    def test_check_offer_lessee_admin(self, mock_gppit, mock_authorize):
        ...
        utils.check_offer_lessee(
            admin_ctx.to_policy_values(), test_offer_lessee_no_match
        )
    ...
    def test_check_offer_lessee_non_admin_match(self, mock_gppit, mock_authorize):
        ...
        utils.check_offer_lessee(lessee_ctx.to_policy_values(), test_offer_lessee_match)
...
class TestPolicyAuthorizeUtils(testtools.TestCase):
    ...
    def test_policy_authorize_exception(self, mock_authorize):
        mock_authorize.side_effect = oslo_policy.PolicyNotAuthorized(
            "esi_leap:offer:offer_admin", lessee_ctx.to_dict(), lessee_ctx.to_dict()
        )
    ...
    def test_resource_policy_authorize_exception(self, mock_authorize):
        mock_authorize.side_effect = exception.HTTPForbidden("test_policy:test")
...
class TestOfferGetDictWithAddedInfoUtils(testtools.TestCase):
    @mock.patch("esi_leap.common.idp.get_project_name")
...
class TestCheckLeaseLength(testtools.TestCase):
    ...
    def test_check_lease_length_admin_exceed_max(self, mock_authorize):
        utils.check_lease_length(
            admin_ctx.to_policy_values(), self.start_time, self.end_time, self.max_time
        )
    ...
    def test_check_lease_length_exception(self, mock_authorize):
        ...
        self.assertRaises(
            exception.LeaseExceedMaxTimeRange,
            utils.check_lease_length,
            lessee_ctx.to_policy_values(),
            self.start_time,
            self.end_time,
