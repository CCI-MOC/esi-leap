...
class TestResourceObjects(base.TestCase):
    ...
    def setUp(self):
        super(TestResourceObjects, self).setUp()
    ...
    def test_get_class_valid(self):
        ...
        for type_name, expected_type in types.items():
            self.assertEqual(resource_objects.get_class(type_name),
                             expected_type)
    ...
    def test_get_class_invalid(self):
        ...
        for type_name in invalid_types:
            self.assertRaises(exception.ResourceTypeUnknown,
                              resource_objects.get_class,
                              type_name)

    @mock.patch('esi_leap.resource_objects.get_class')
    def test_get_resource_object(self, mock_gc):
        mock_type = mock.MagicMock()
        mock_type.return_value = 'i have data!'
        mock_gc.return_value = mock_type
        ...
        mock_gc.assert_called_once_with('fake_node')
